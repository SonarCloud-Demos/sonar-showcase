...
public class DataImportController {
    ...
    @Operation(
        ...
        description = "Deserializes Base64-encoded serialized data with class filtering to prevent unsafe deserialization."
    )
    ...
    /**
     * A filtered ObjectInputStream that only allows pre-approved classes
     * to be deserialized, preventing insecure deserialization attacks.
     */
    ...
    public ResponseEntity<String> restoreSession(
        ...
        try {
            ...
            ObjectInputStream ois = new ObjectInputStream(
                ...
                @Override
                protected Class<?> resolveClass(ObjectStreamClass osc) throws IOException, ClassNotFoundException {
                    if (!java.lang.String.class.getName().equals(osc.getName())) {
                        throw new InvalidClassException("Unauthorized deserialization", osc.getName());
                    }
                    return super.resolveClass(osc);
                }
